        ...
        public void DoAuto(){
            do{
                ...
                if (IsBIT[B.Stage1PickUp]){
                    B.SetBit(nThread, B.Stage1PickUp, false, "맵-블록 테이블1 픽업 중 홈 진행하여 이여서 작업 진행");
                    B.SetBit(nThread, B.StageAirshowerWait, true, "맵-블록 테이블 에어사워 일시 정지 플로그 ON");
                }
            RePLACE:
                while (B.WaitBIT(nThread, B.Stage1_UnitReceive, false, "유닛 공급 대기")){
                    if (!bBD) {
            ...
            GoWORK:
                ...
                while (B.WaitBIT(nThread, B.Stage2Inspection, true, "맵-블록 테이블2 유닛 검사 진행 중이여 대기")) ;
                B.SetBit(nThread, B.Stage1Inspection, true, "맵-블록 테이블1 유닛 검사 진행 플로그");
                if (eRTN.SUCESS != UnitInspection(nThread, eMAP_BLOCK.STAGE1, "맵-블록 테이블1 유닛 검사 진행")){
                    StageWorkCencel(nThread, eMAP_BLOCK.STAGE1, "맵-블록1 테이블 작업 취소 됨");
                    UTIL_.DELAY(500);
                    goto RePLACE;
                }

                while (B.WaitBIT(nThread, B.Stage2Busy, true, "맵-블록 테이블2 유닛 픽업 작업 중 대기")){
                ...
                B.SetBit(nThread, B.Stage1Busy, true, "테이블1번 픽업 작업 진행");
                ...
                UTIL_.DELAY(1000);
                LogWR_.SavePCBUnitInfo(nThread, eMAP_BLOCK.STAGE1);
                CLOT.RESET_STRIP_INFO(eSeqBacode.MapBlock1, nThread);
                B.SetBit(nThread, B.Stage1Working, false, "맵-블록 1 작업 완료");
            } while (true);
        }
    }
}