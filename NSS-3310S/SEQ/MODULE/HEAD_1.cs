    ...
    public class HEAD_1 : BASE{
        ...
        public void DoAuto(){
            ...
            do{
                ...
                while (UTIL_.WaitBIT(nThread, B.X2PicBusy, true, "X2 픽업 진행 완료할때까지 대기")) ;
                COM_.SetBit(nThread, B.X1PicBusy, true, "X1 픽업 진행 플로그");
                INFO_PICKER_DATA_CLEANER(eHD.HD1, L.CurWorkXPic);
                UnitPic(nThread, eHD.HD1, "X1 유닛 픽업");

                while (UTIL_.WaitBIT(nThread, B.X2PRSBusy, true, "HEAD X2 PRS 작업 진행 중 완료할때까지 대기")) ;
                COM_.SetBit(nThread, B.X1PRSBusy, true, "HEAD X1 PRS 작업 진행");
                PRS(nThread, eHD.HD1, "X1 PRS 검사");

                while (UTIL_.WaitBIT(nThread, B.X2PlcBusy, true, "HEAD X2 플레이스 진행 완료할때까지 대기")) ;
                COM_.SetBit(nThread, B.X1PlcBusy, true, "X1 플레이스 진행 플로그");
                UnitPlc(nThread, eHD.HD1, "X1 유닛 플레이스");

                while (UTIL_.WaitBIT(nThread, B.X2NGPlcBusy, true, "HEAD X2 NG 유닛 플레이스 진행 완료 할때까지 대기")) ;
                COM_.SetBit(nThread, B.X1NGPlcBusy, true, "X1 NG유닛 플레이스 진행 플로그");
